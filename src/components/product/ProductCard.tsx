  ...
  return (
    <div className="bg-white rounded-2xl p-3 shadow-xs border border-amber-900/5 hover:border-[#F2AF5B]/50 hover:shadow-md transition-all flex flex-col h-full relative group overflow-hidden">
      ...
      <div className="flex-1 flex flex-col z-10 select-none">
        ...
        {/* Title */}
        <h3 className="font-extrabold text-stone-900 text-sm md:text-base leading-tight mb-1 group-hover:text-[#4A2E1B] transition-colors line-clamp-1">
          {resolvedName}
        </h3>
        
        {/* Sub-description snippet */}
        <p className="text-xs text-stone-500 line-clamp-2 mb-3 font-normal leading-relaxed">
          {resolvedDesc}
        </p>
        
        {/* Custom Sales Count Indicator as requested! */}
        {product.customSalesCount && product.customSalesCount > 0 ? (
          <div className="flex items-center gap-1 bg-amber-50 rounded-lg py-1 px-2 mb-3 border border-amber-100 w-fit">
            <Flame size={12} className="text-amber-600 fill-amber-500" />
            <span className="text-[9px] md:text-[10px] font-bold text-amber-900">
        ...
        <div className="mt-auto flex items-center justify-between gap-2 overflow-hidden">
          ...
          <div className="flex flex-col">
            ...
            {isDiscounted && (
              <span className="text-[11px] text-stone-400 font-bold line-through -mb-0.5">
                ৳{(product.price || 0).toLocaleString()}
              </span>
            )}
            <span className="font-black text-stone-900 text-base md:text-lg">
              ৳{(displayPrice || 0).toLocaleString()}
            </span>
          </div>

          {/* Elegant Circular Bag button from Reference Image */}
          <button 
            onClick={handleAddToCart}
            className="relative z-30 w-10 h-10 rounded-full bg-[#FAF6F0] hover:bg-[#F2AF5B] border border-amber-900/10 hover:border-transparent text-[#5C3E21] hover:text-amber-950 flex items-center justify-center shadow-xs transition-all cursor-pointer grow-0 shrink-0 select-none focus:outline-none focus:ring-2 focus:ring-[#F2AF5B]"
