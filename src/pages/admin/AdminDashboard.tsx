...
export default function AdminDashboard() {
  ...
  // Active Tab: 'overview' | 'products' | 'orders' | 'roles' | 'coupons' | 'settings'
  const [activeTab, setActiveTab] = useState<'overview' | 'products' | 'orders' | 'roles' | 'coupons' | 'settings'>('overview');
  ...
  const [emailTestResult, setEmailTestResult] = useState<{ success: boolean; message?: string; error?: string; hint?: string } | null>(null);
  ...
  const fetchCoupons = async () => {
    ...
    try {
      ...
      const snapshot = await getDocs(q);
      const couponList = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
  ...
  const fetchSettings = async () => {
    try {
      ...
      if (docSnap.exists()) {
        ...
        setInsideDhakaCharge(data.insideDhakaCharge || 70);
        setOutsideDhakaCharge(data.outsideDhakaCharge || 130);
      }
    } catch (e) {
  ...
  const handleSaveSettings = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(language === 'bn' ? 'সেটিংস সেভ হয়েছে!' : 'Settings saved successfully!');
    } catch (e) {
  ...
  const handleTestEmailConnection = async () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        toast.success(language === 'bn' ? 'কানেকশন সফল! টেস্ট ইমেইল পাঠানো হয়েছে।' : 'Connection successful! Test email sent.');
      } else {
        setEmailTestResult({
          success: false,
        ...
        toast.error(language === 'bn' ? 'ইমেইল কানেকশন ব্যর্থ হয়েছে!' : 'Email connection failed!');
      }
    } catch (err: any) {
      setEmailTestResult({
        success: false,
      ...
      toast.error(language === 'bn' ? 'নেটওয়ার্ক এরর!' : 'Network error!');
    } finally {
  ...
  const handleCreateCoupon = async (e: React.FormEvent) => {
    ...
    if (!newCouponCode.trim() || !newCouponDiscount) {
      toast.error(language === 'bn' ? "কুপন কোড এবং ছাড়ের পরিমাণ দিন।" : "Please provide coupon code and discount amount.");
      return;
    }
    setLoading(true);
    try {
      ...
      toast.success(language === 'bn' ? "কুপন তৈরি হয়েছে!" : "Coupon created successfully!");
      setNewCouponCode('');
  ...
  const handleToggleCoupon = async (couponId: string, currentStatus: boolean) => {
    try {
      ...
      toast.success(language === 'bn' ? "কুপন আপডেট হয়েছে!" : "Coupon status updated!");
      fetchCoupons();
    } catch (err) {
  ...
  const handleDeleteCoupon = async (couponId: string) => {
    if (!window.confirm(language === 'bn' ? "আপনি কি নিশ্চিত কুপনটি ডিলিট করতে চান?" : "Are you sure you want to delete this coupon?")) return;
    try {
      await deleteDoc(doc(db, 'coupons', couponId));
      toast.success(language === 'bn' ? "কুপন ডিলিট হয়েছে!" : "Coupon deleted!");
      fetchCoupons();
    } catch (err) {
      ...
      <div className="flex-1 max-w-7xl w-full mx-auto p-4 md:py-8">
        ...
        {activeTab === 'settings' && (
          <div className="space-y-8 animate-fade-in">
            <div className="bg-white p-6 md:p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-amber-900/5 max-w-3xl">
              <div className="flex items-center gap-3 mb-6">
                <div className="p-3 bg-stone-100 rounded-xl text-stone-700">
                  <Settings size={20} />
                </div>
                <div>
                  <h3 className="text-xl font-black text-stone-800">{language === 'bn' ? 'স্টোর সেটিংস' : 'Store Settings'}</h3>
                  <p className="text-sm text-stone-500 font-medium">Configure shipping and delivery charges.</p>
                </div>
              </div>
              ...
              <form onSubmit={handleSaveSettings} className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 bg-stone-50 p-6 rounded-2xl border border-stone-100">
                  <div>
                    <label className="block text-xs font-bold text-stone-600 mb-2 uppercase tracking-wide">
                      {language === 'bn' ? 'ঢাকায় ডেলিভারি চার্জ (৳)' : 'Inside Dhaka Charge (৳)'}
                    </label>
                    <input
                      type="number"
                      required
                      ...
                      className="w-full border-2 border-stone-200 rounded-xl px-4 py-3 focus:outline-none focus:border-[#593A1B] font-bold text-sm tracking-wide"
                      min="0"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-stone-600 mb-2 uppercase tracking-wide">
                      {language === 'bn' ? 'ঢাকার বাইরে ডেলিভারি চার্জ (৳)' : 'Outside Dhaka Charge (৳)'}
                    </label>
                    <input
                      type="number"
                      required
                ...
                <div className="flex justify-end border-t border-stone-100 pt-6">
                  <button
                    ...
                    className="bg-[#3E2511] hover:bg-[#201105] text-white font-extrabold text-sm py-3.5 px-8 rounded-xl transition-all shadow-md uppercase tracking-wider flex justify-center items-center gap-2"
                  >
                    {settingsLoading ? <RefreshCw className="animate-spin" size={18} /> : <Save size={18} />}
            ...
            {/* Email Notification & SMTP Diagnostics Card */}
            <div className="bg-white p-6 md:p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-amber-900/5 max-w-3xl mt-8">
              <div className="flex items-center gap-3 mb-6">
                <div className="p-3 bg-stone-100 rounded-xl text-stone-700">
                  <Mail size={20} />
                </div>
                <div>
                  <h3 className="text-xl font-black text-stone-800">
                    {language === 'bn' ? 'ইমেইল সিস্টেম ডায়াগনস্টিকস' : 'Email Notification & SMTP Diagnostics'}
                  </h3>
                  <p className="text-sm text-stone-500 font-medium">
                    {language === 'bn' ? 'অর্ডার কনফার্মেশন ইমেইল সিস্টেম পরীক্ষা করুন।' : 'Verify and test the order confirmation email configuration.'}
                  </p>
                </div>
              </div>
              ...
              <div className="bg-stone-50 p-6 rounded-2xl border border-stone-100 space-y-4">
                <div className="space-y-2">
                  <p className="text-xs font-bold text-stone-600 uppercase tracking-wide">
                    {language === 'bn' ? 'বর্তমান ইমেইল সেটিংস স্থিতি' : 'CURRENT CREDENTIALS STATUS'}
                  </p>
                  <p className="text-sm font-semibold text-stone-700">
                    Gmail address (EMAIL_USER): <span className="font-mono text-stone-900 bg-stone-200/50 px-2 py-0.5 rounded text-xs select-all">talhaxqutex@gmail.com</span>
                  </p>
                  <p className="text-sm font-semibold text-stone-700">
                    App Password (EMAIL_PASS): <span className="font-mono text-stone-900 bg-stone-200/50 px-2 py-0.5 rounded text-xs">•••••••••••••••• (Configured)</span>
                  </p>
                </div>
                ...
                <div className="border-t border-stone-200/60 pt-4">
                  <button
                    ...
                    className="w-full sm:w-auto bg-stone-900 hover:bg-[#201105] text-white font-extrabold text-sm py-3.5 px-6 rounded-xl transition-all shadow-md uppercase tracking-wider flex justify-center items-center gap-2 disabled:opacity-50 cursor-pointer"
                  >
                    {emailTesting ? <RefreshCw className="animate-spin" size={18} /> : <Send size={18} />}
                    {language === 'bn' ? 'কানেকশন টেস্ট করুন' : 'Test SMTP Connection'}
                  </button>
                </div>
                ...
                {emailTestResult && (
                  <div className={`mt-4 p-4 rounded-xl border ${
                    ...
                    <h4 className="font-bold text-sm mb-1 flex items-center gap-2">
                      {emailTestResult.success ? (
                        <>🎉 {language === 'bn' ? 'পরীক্ষা সফল হয়েছে!' : 'Test Successful!'}</>
                      ) : (
                        <>⚠️ {language === 'bn' ? 'পরীক্ষা ব্যর্থ হয়েছে' : 'Verification Failed'}</>
                      )}
                    </h4>
                    ...
                    {!emailTestResult.success && emailTestResult.hint && (
                      <div className="mt-2 text-xs border-t border-rose-200/60 pt-2 text-rose-700 font-semibold space-y-1">
                        <p className="uppercase font-bold text-[10px] tracking-wider text-rose-900">Recommended Steps:</p>
                        <p>{emailTestResult.hint}</p>
                      </div>
                    )}
                  </div>
                )}
                      ...
                      {isExpanded && (
                        <div className="border-t border-stone-100 p-5 bg-stone-50/50 text-xs text-stone-700 leading-relaxed grid grid-cols-1 lg:grid-cols-12 gap-6 text-left">
                          ...
                          <div className="lg:col-span-6 space-y-3">
                            ...
                            {order.couponCode && (
                              <div className="bg-emerald-50 text-emerald-800 p-3 rounded-xl border border-emerald-100 flex justify-between items-center text-xs">
                                <div>
                                  <span className="font-bold uppercase tracking-wide">Applied Coupon: </span>
                                  <span className="font-mono font-black">{order.couponCode}</span>
                                </div>
                          ...
                          <div className="lg:col-span-3 space-y-2">
                            ...
                            <div className="bg-white p-4 rounded-xl border border-stone-100 space-y-1.5 text-[11px] text-stone-600">
                              ...
                              {order.deliveryArea && (
                                <p className="font-bold text-stone-700 mt-1">
                                  Selected Area: <span className="bg-amber-50 px-2 py-0.5 rounded text-amber-950 border border-amber-200 capitalize font-extrabold">{order.deliveryArea === 'inside' ? 'Inside Dhaka' : 'Outside Dhaka'}</span>
                                </p>
                              )}
                            ...
                            <div className="bg-white p-4 rounded-xl border border-stone-100 space-y-3.5 text-[11px]">
                              ...
                              <div className="flex gap-4 flex-wrap">
                                ...
                                {order.shippingCharge !== undefined && (
                                  <div>
                                    <span className="text-[10px] font-bold uppercase text-stone-400 block mb-1">Deliv Charge</span>
                                    <span className="bg-stone-100 text-[#482b13] px-2 py-1.5 rounded font-black text-xs inline-block tracking-wider border border-stone-200">
              ...
              {usersLoading ? (
                ...
                <div className="overflow-x-auto rounded-2xl border border-stone-150">
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-stone-50 text-xs font-semibold">
                      {usersList.map((usr, i) => {
                        const isThisOwner = usr.email === 'talhaahmed13728@gmail.com' || usr.email === 'talhaxqutex@gmail.com';
        ...
        {activeTab === 'coupons' && (
          <div className="space-y-8 animate-fade-in">
            <div className="bg-white p-6 md:p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-amber-900/5">
              <div className="flex items-center gap-3 mb-6">
                <div className="p-3 bg-[#593A1B]/10 rounded-xl text-[#593A1B]">
                  <Tag size={20} />
                </div>
                <div>
                  <h3 className="text-xl font-black text-[#593A1B]">{language === 'bn' ? 'কুপন সিস্টেম' : 'Coupon Management'}</h3>
                  <p className="text-sm text-stone-500 font-medium">Create and distribute promotional codes for instant discounts.</p>
                </div>
              </div>

              <form onSubmit={handleCreateCoupon} className="grid grid-cols-1 md:grid-cols-12 gap-4 mb-8 bg-stone-50 p-6 rounded-2xl border border-stone-100">
                <div className="md:col-span-5">
                  <label className="block text-xs font-bold text-stone-600 mb-2 uppercase tracking-wide">
                    {language === 'bn' ? 'কুপন কোড (যেমন: EID2025)' : 'Coupon Code'}
                  </label>
                  <input
                    type="text"
                    required
                    ...
                    className="w-full border-2 border-stone-200 rounded-xl px-4 py-3 focus:outline-none focus:border-[#593A1B] font-mono text-sm tracking-wider uppercase font-bold"
                    placeholder="e.g., SUMMER50"
                  />
                </div>
                <div className="md:col-span-4">
                  <label className="block text-xs font-bold text-stone-600 mb-2 uppercase tracking-wide">
                    {language === 'bn' ? 'ছাড়ের পরিমাণ (৳)' : 'Discount Amount (৳)'}
                  </label>
                  <input
                    type="number"
                    required
                ...
                <div className="md:col-span-3 flex items-end">
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full bg-[#3E2511] hover:bg-[#201105] text-white font-extrabold text-sm py-3.5 px-4 rounded-xl transition-all shadow-md uppercase tracking-wider flex justify-center items-center gap-2"
                  >
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-stone-50 border-b border-stone-200">
                        <th className="p-4 text-xs font-black uppercase text-stone-500 tracking-wider">Coupon Code</th>
                        <th className="p-4 text-xs font-black uppercase text-stone-500 tracking-wider">Discount</th>
                        <th className="p-4 text-xs font-black uppercase text-stone-500 tracking-wider">Status</th>
                        <th className="p-4 text-xs font-black uppercase text-stone-500 tracking-wider text-right">Actions</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-stone-100">
                      {couponsLoading ? (
                        <tr>
                          <td colSpan={4} className="p-8 text-center text-stone-400">Loading coupons...</td>
                        </tr>
                      ) : coupons.length === 0 ? (
                        <tr>
                          <td colSpan={4} className="p-8 text-center text-stone-400 font-bold bg-stone-50/50">
                            {language === 'bn' ? 'কোনো কুপন তৈরি করা হয়নি।' : 'No coupons exist yet.'}
                          </td>
                        </tr>
                      ) : (
                        coupons.map((coupon) => (
                          <tr key={coupon.id} className="hover:bg-stone-50/50 transition-colors group">
                            <td className="p-4">
                              <span className="font-mono text-sm font-black bg-emerald-50 text-emerald-800 px-3 py-1.5 rounded-lg border border-emerald-200">
                                {coupon.code}
                              </span>
                            </td>
                            <td className="p-4 font-bold text-lg text-rose-600">
                            ...
                            <td className="p-4">
                              ...
                              >
                                {coupon.isActive ? (language === 'bn' ? 'চালু' : 'Active') : (language === 'bn' ? 'বন্ধ' : 'Disabled')}
                              </button>
                            </td>
